...
// Collect formData for Strapi file uploads with json data
export const collectFormData = function(form, isFiles = true) {
  ...
  for (let i = 0; i < formElements.length; i++) {
    ...
    if (!['submit', 'file'].includes(currentElement.type)) {
      if (currentElement.name === 'tags' && !data[currentElement.name]) {
        data[currentElement.name] = [currentElement.value];
      } else if (currentElement.name === 'tags' && data[currentElement.name]) {
        data[currentElement.name] = [...data[currentElement.name], currentElement.value];
      } else {
        data[currentElement.name] = currentElement.value;
      };
    } else if (currentElement.type === 'file' && isFiles) {
