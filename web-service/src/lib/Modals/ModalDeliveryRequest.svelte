    ...
    if (button.disabled === false) {
      ...
      const sendMailRes = await fetch(sendMailUrl, {
        ...
        body: JSON.stringify({
          ...
          subject: 'Заявка на поставку продукции с сайта ООО "Промресурс"',
          text: `
Здравствуйте!\n
Вам отправлено сообщение из формы "Заявка на поставку продукции" (открыто в "${$modalData.header} / ${$modalData.button.text}")\n
...
Текст сообщения: ${message}
          ...
          html: `
            <h2>Здравствуйте!</h2>
            <p>Вам отправлено сообщение из формы <b>"Заявка на поставку продукции"</b> (открыто в "${$modalData.header} / ${$modalData.button.text}")</p>
...
{#if $modalData}
  <div class="wrapper">
    ...
    <form on:submit={sendData}>
      ...
      <textarea bind:value={message} placeholder="Необходимая продукция" required></textarea>
