    ...
    if (button.disabled === false) {
      ...
      const sendMailRes = await fetch(sendMailUrl, {
        ...
        body: JSON.stringify({
          ...
          subject: 'Запрос на подписку по номеру телефона с сайта ООО "Промресурс"',
          text: `
Здравствуйте!\n
Вам отправлено сообщение из формы "Связаться с нами" (открыто в "${$modalData.header} / ${$modalData.button.text}")\n
Имя отправителя: ${name}\n
Номер телефона отправителя: ${phone}
          `,
          html: `
            <h2>Здравствуйте!</h2>
            <p>Вам отправлено сообщение из формы <b>"Связаться с нами"</b> (открыто в "${$modalData.header} / ${$modalData.button.text}")</p>
...
{#if $modalData}
  <div class="wrapper">
    <h2>
      Подписаться на рассылку предложений
    </h2>
