    ...
    if (button.disabled === false) {
      ...
      const sendMailRes = await fetch(sendMailUrl, {
        ...
        body: JSON.stringify({
          ...
          text: `
Здравствуйте!\n
Вам отправлено сообщение из формы "Подписка на акции и скидки" (открыто в "${$modalData.header} / ${$modalData.button.text}")\n
...
Номер телефона отправителя: ${phone}
          ...
          html: `
            <h2>Здравствуйте!</h2>
            <p>Вам отправлено сообщение из формы <b>"Подписка на акции и скидки"</b> (открыто в "${$modalData.header} / ${$modalData.button.text}")</p>
